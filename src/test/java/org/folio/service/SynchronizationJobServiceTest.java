...
public class SynchronizationJobServiceTest extends TestBase {
  ...
  void shouldProcessUserSummariesInBatches() {
    int userCount = 25; // 20 (batch size) + 5 (last batch)
    ...
    List<List<String>> batches = List.of(
      captor.getAllValues().subList(0, 20), // first batch
      captor.getAllValues().subList(20, 25) // last batch
    );
    assertEquals(20, batches.get(0).size(), "First batch size should be 20");
